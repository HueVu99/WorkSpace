...
cover.addEventListener("mousemove", function(e){
  ...
  let y = e.pageY - screenImage.offsetParent ?.offsetLeft-screenImage.offsetTop;
  
  const positionX =( imageWidth /screenWidth / 2) * x;
  const positionY =( imageWidth /screenWidth / 2) * y;

  image.style = 'left: ${-positionX}px;top: ${-positionY}px;width: auto;height: unset;transform: none;';

});
cover.addEventListener("mouseleave", function(e){
  const image =document.querySelector(".screen-img__image");
  image.style="";
})